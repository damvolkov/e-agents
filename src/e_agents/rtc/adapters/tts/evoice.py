"""TTS adapter for e-voice via HTTP chunked streaming."""
...
class EVoiceChunkedStream(tts.ChunkedStream):
    """Stream raw binary PCM from e-voice /audio/speech via HTTP chunked transfer."""

    def __init__(
        self,
        *,
        tts_instance: "EVoiceTTS",
        input_text: str,
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        """Stream chunked binary PCM from e-voice."""
        tts_instance: EVoiceTTS = self._tts  # type: ignore[assignment]
...
class EVoiceTTS(tts.TTS):
    """TTS adapter for e-voice via HTTP chunked streaming."""

    def __init__(
        self,
        *,
        base_url: str = str(st.TTS_BASE_URL),
    ...
    def synthesize(
        ...
        *,
        ...
        """Synthesize text to streaming PCM audio via e-voice."""
        return EVoiceChunkedStream(
            tts_instance=self,
            input_text=text,
